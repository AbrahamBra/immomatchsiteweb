    ...
    <section id="fonctionnement" className="bg-[#080808] text-white py-32 px-6 border-t border-white/10">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-gray-400 text-lg max-w-xl leading-relaxed mb-24">
          De la qualification à la proposition — Zefir automatise la chaîne de valeur
          complète, sans friction pour l'agent.
        </p>
        ...
        <div className="flex flex-col gap-0">
          {STEPS.map((step) => (
            <div
              key={step.number}
              className="grid grid-cols-1 lg:grid-cols-[120px_1fr] gap-0 border-t border-white/10 py-12 group"
            >
              {/* Number */}
                    ...
                    <div className="mt-8 flex flex-col gap-4">
                      {step.subSteps.map((sub) => (
                        <div key={sub.label} className="flex gap-4 items-start">
                          <div className="w-px h-full bg-white/20 self-stretch min-h-[24px] mt-1.5 ml-1" />
                          <div>
